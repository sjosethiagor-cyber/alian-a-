...
export default function PrayerStudy({ items = [], onToggle, onAdd, onAddVideo, refreshTrigger }: PrayerStudyProps) {
    const completedCount = items.filter(i => i.completed).length;

    // Helper for avatar/icon in checklist (removed unused)
        ...
        <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto', color: 'white' }}>
            ...
            <section style={{ margin: '24px 0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    <h3 style={{ fontSize: '1.2rem', fontWeight: 700 }}>Checklist Diário</h3>
                    <div style={{
                        ...
                        {completedCount}/{Math.max(items.length, 3)} Concluído
                    </div>
                </div>

                {/* Optional: Stats Chart requested by User, integrated near checklist */}
                <PrayerStats total={items.length || 3} completed={completedCount} />

                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {items.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '20px', color: '#64748b' }}>
                            <p>Nenhuma oração listada para hoje.</p>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem', color: '#64748b' }}>
                                        {item.meta ? (
                                            ...
                                            <span>Manhã • Concluído às --:--</span>
                                        )}
                                    </div>
                                </div>

                                {/* Avatar/Person Indicator */}
                                <div style={{
